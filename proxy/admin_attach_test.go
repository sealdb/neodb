...
func TestAttachAndListAndDetach(t *testing.T) {
	...
	query := fmt.Sprintf("neodb attach('%s', 'mock', 'pwd')", backend1)
	node, err := sqlparser.Parse(query)
	assert.Nil(t, err)
	attach := node.(*sqlparser.NeoDB)
	_, err = handler.Attach(attach)
	assert.Nil(t, err)

	query = fmt.Sprintf("neodb attachlist")
	node, err = sqlparser.Parse(query)
	assert.Nil(t, err)
	_, err = handler.ListAttach()
	assert.Nil(t, err)

	query = fmt.Sprintf("neodb detach('attach1')")
	node, err = sqlparser.Parse(query)
	assert.Nil(t, err)
...
func TestDetachError(t *testing.T) {
	...
	attachName := "attach1"
	query := fmt.Sprintf("neodb Detach('%s');", attachName)
...
func TestAttachErrorParams(t *testing.T) {
	...
	query := fmt.Sprintf("neodb attach('attach1');")
	node, err := sqlparser.Parse(query)
	assert.Nil(t, err)
	attach := node.(*sqlparser.NeoDB)
	_, err = handler.Attach(attach)
...
func TestAttachErrorDuplicateBackend(t *testing.T) {
	...
	query := fmt.Sprintf("neodb attach('backend1', 'mock', 'pwd')")
	node, err := sqlparser.Parse(query)
	assert.Nil(t, err)
	attach := node.(*sqlparser.NeoDB)
	_, err = handler.Attach(attach)
...
func TestAttachErrorDuplicateAddress(t *testing.T) {
	...
	query := fmt.Sprintf("neodb attach('%s', 'mock', 'pwd')", addrs[0])
	node, err := sqlparser.Parse(query)
	assert.Nil(t, err)
	attach := node.(*sqlparser.NeoDB)
	_, err = handler.Attach(attach)
...
func TestAttachErrorRouteCreateTable(t *testing.T) {
	...
	backend2 := addrs[2]
	query = fmt.Sprintf("neodb attach('%s', 'mock', 'pwd')", backend2)
	node, err = sqlparser.Parse(query)
	assert.Nil(t, err)
	attach = node.(*sqlparser.NeoDB)
	_, err = handler.Attach(attach)
