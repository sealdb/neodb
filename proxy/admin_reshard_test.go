...
func TestReshardMockShiftLow(t *testing.T) {
	...
	// neodb reshard failed, check.
	{
		query := "neodb reshard test1.s to test1.b"
	...
	// neodb reshard failed, check.
	{
		query := "neodb reshard test.s to test1.b"
		_, err := sqlparser.Parse(query)
		assert.Nil(t, err)
	...
	// neodb reshard failed, check.
	{
		query := "neodb reshard test.a to test1.b"
		_, err := sqlparser.Parse(query)
		assert.Nil(t, err)
	...
	// neodb reshard failed, check the same table.
	{
		reshard := NewReshard(log, scatter, router, spanner, "mock")
		th := &TestHandler{address: address}
		reshard.SetHandle(th)
	...
	// neodb reshard failed.
	{
		query := "neodb reshard test.a to test.b"
		_, err := sqlparser.Parse(query)
		assert.Nil(t, err)
