...
func (attach *Attach) Attach(node *sqlparser.NeoDB) (*sqltypes.Result, error) {
	row := node.Row
	log := attach.log
	scatter := attach.scatter

	if len(row) != AttachParamsCount {
		return nil, errors.Errorf("spanner.query.execute.neodb.%s.error,", node.Action)
	}
	var p attachParams
...
func (attach *Attach) addAttachHandler(log *xlog.Log, scatter *backend.Scatter, p *attachParams) (err error) {
	...
	// Sync table on the attach backend to neodb's router as single table
	// just register the info on the neodb, it wll be fast.
...
// check the table valid by creating table on the neodb to .
