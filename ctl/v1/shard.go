...
import (
	...
	shiftlog "github.com/sealdb/neodb/tools/shift/xlog"
)
...
func shardRuleShiftHandler(log *xlog.Log, proxy *proxy.Proxy, w rest.ResponseWriter, r *rest.Request) {
	...
	if err != nil {
		log.Error("api.v1.neodb.shard.rule.parse.json.error:%+v", err)
		rest.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	log.Warning("api.v1.neodb.shard.rule[from:%v].request:%+v", r.RemoteAddr, p)
