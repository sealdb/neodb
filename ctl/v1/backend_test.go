...
func TestCtlV1BackendAdd(t *testing.T) {
...

	{
		...
		recorded := test.RunRequest(t, handler, test.MakeSimpleRequest("POST", "http://localhost/v1/neodb/backend", p))
		recorded.CodeIs(200)
	}
...
func TestCtlV1BackendAddError(t *testing.T) {
	...
	// 500.
	{
		recorded := test.RunRequest(t, handler, test.MakeSimpleRequest("POST", "http://localhost/v1/neodb/backend", nil))
		recorded.CodeIs(500)
	}
...
func TestCtlV1BackendAddInitBackend(t *testing.T) {
	...
	recorded := test.RunRequest(t, handler, test.MakeSimpleRequest("POST", "http://localhost/v1/neodb/backend", p1))
	recorded.CodeIs(200)
	...
	recorded = test.RunRequest(t, handler, test.MakeSimpleRequest("POST", "http://localhost/v1/neodb/backend", p2))
	recorded.CodeIs(500)
}
...
func TestCtlV1BackendRemove(t *testing.T) {
	...
	api := rest.NewApi()
	router, _ := rest.MakeRouter(
		rest.Delete("/v1/neodb/backend/:name", RemoveBackendHandler(log, proxy)),
	)
	api.SetApp(router)
	handler := api.MakeHandler()

	{
		recorded := test.RunRequest(t, handler, test.MakeSimpleRequest("DELETE", "http://localhost/v1/neodb/backend/backend1", nil))
		recorded.CodeIs(200)
	}
}
...
func TestCtlV1BackendRemoveError(t *testing.T) {
	...
	// 404.
	{
		recorded := test.RunRequest(t, handler, test.MakeSimpleRequest("DELETE", "http://localhost/v1/neodb/backend/xx", nil))
		recorded.CodeIs(500)
	}
}
