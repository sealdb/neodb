...
var (
	toFlavor = flag.String("to-flavor", "mysql", "Destination db flavor, like mysql/mariadb/neodb")
	...
	neoDBURL               = flag.String("neodburl", "http://127.0.0.1:8080", "NeoDB RESTful api(defaults http://127.0.0.1:8080)")
...
func check(log *xlog.Log) {
	if *toFlavor == "" || *from == "" || *fromUser == "" || *fromDatabase == "" || *fromTable == "" ||
		*to == "" || *toUser == "" || *toDatabase == "" || *toTable == "" {
		log.Panic("usage: shift --from=[host:port] --from-database=[database] --from-table=[table] --from-user=[user] --from-password=[password] --to=[host:port] --to-database=[database] --to-table=[table]  --to-user=[user] --to-password=[password] --cleanup=[false|true] --to-flavor=[mysql|neodb|mariadb]")
	}
}
